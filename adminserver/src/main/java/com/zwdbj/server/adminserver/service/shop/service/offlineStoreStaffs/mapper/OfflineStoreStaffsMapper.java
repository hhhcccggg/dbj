...
public interface OfflineStoreStaffsMapper {
    ...
    @Select("SELECT u.fullName,u.phone,u.id,u.createTime,u.tenantId,u.notes," +
            "if((select count(1) from o2o_offlineStoreStaffs as os where storeId=#{storeId} and isDeleted=0 and  os.userId=u.id)>0,1,0) as isSuperStar " +
            "from core_users as u " +
            " where u.tenantId=#{tenantId} and u.isDeleted=0 and u.id=#{id}")
