...
public interface NearbyShopsMapper {
    @Select("select id,name,logoUrl,grade,subName,address,mainConverImage,coverImages from shop_stores where id=#{storeId}")
    ...
    List<ShopServices> searchServiceScopes(@PathVariable("storeId") long storeId);
    ...
    @Select("select c.id,c.name from core_categories as c,o2o_offlineExtraServices as o where " +
            " o.storeId=#{storeId} and c.id=o.serviceScopeId ")
    List<ShopServices> searchExtraServices(@PathVariable("storeId") long storeId);

    @Select("select day,openingTime,closeTime from o2o_offlineStoreOpeningHours where storeId=#{storeId}")
