...
public interface IProductOrderMapper {
    ...
    @Insert("insert into shop_productOrders(id,orderNo,payment,actualPayment,useCoin,paymentType,thirdPaymentTradeNo," +
            ...
            "values(#{id},#{id},#{payment},0,#{input.useCoin},'NONE','NONE',0,'STATE_BUYER_PAYED',now(),now()," +
            "#{userId},#{input.storeId},#{input.buyerComment},0,#{input.receiveAddressId})")
    ...
    @Select("select sum(oi.num) from shop_productOrderItems oi left join shop_productOrder o on o.id=oi.orderId " +
