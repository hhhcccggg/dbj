...
public interface ShopCommentsMapper {
    ...
    @Select("SELECT u.id as userId ,u.userName,u.avatarUrl,c.contentTxt,c.heartCount,c.createTime,p.name as resourceName," +
            "c.resourceOwnerId,c.resourceTypeId,c.rate,ce.commentId,ce.type,ce.dataId,ce.dataContent " +
            "FROM core_comments as c,core_comment_extraDatas as ce,core_users as u,shop_products as p " +
            "WHERE c.resourceTypeId=1 or c.resourceTypeId=2,p.storeId=#{storeId} and c.resourceOwnerId =p.id and c.userId=u.id and ce.commentId=c.id and c.isOwner=1 order by c.createTime")
